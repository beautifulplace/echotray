#!/usr/bin/env python3
"""WhisperType — click the tray microphone to dictate text at your cursor.
...
whispertype-helperd daemon. The GUI runs as an unprivileged user with no special
groups and talks to the daemon over /run/whispertype.sock.
"""
...
_LOG_PATH = pathlib.Path(os.getenv("WHISPER_LOG", "/tmp/whispertype.log"))
...
def show_error_dialog(message):
    ...
    dialog = Gtk.MessageDialog(
        ...
        text="WhisperType failed to start",
    )
...
_LOCK_PATH = "/tmp/whispertype.lock"


def _acquire_single_instance() -> bool:
    """Ensure only one WhisperType instance runs. Returns True if we got the lock."""
    try:
...
def show_download_progress_dialog(model_size):
    ...
    """
    dialog = Gtk.Dialog(
        title="WhisperType — downloading model",
        transient_for=None,
        flags=0,
    )
...
def check_prerequisites():
    ...
    try:
        ...
        errors.append(
            "The WhisperType helper daemon is not running:\n"
            f"    {e}\n\n"
            "Start it with:\n"
            "    sudo systemctl start whispertype-helperd\n"
            "or re-run setup.sh."
        )
...
class DictationApp:
    def __init__(self, model):
        ...
        # Toggle recording from the menu too.
    ...
    def _on_about(self, _widget):
        ...
        dialog.set_program_name("WhisperType")
        dialog.set_version(__version__)
        # Use the same icon as the desktop launcher for the logo (placeholder
        # until a dedicated logo is designed). Scale it down — the source SVG is
        # 512px and would fill the whole dialog on small screens.
        _LOGO = str(pathlib.Path(__file__).resolve().parent.parent / "assets" / "icon-dark.svg")
        ...
        dialog.set_comments(
            ...
            "Paste (Ctrl+V) handled by the root-owned whispertype-helperd,\n"
            "so WhisperType runs unprivileged with no special groups."
        )
    ...
    def run(self):
        ...
        print("\nReady. Click the round tray microphone to start/stop dictation.\n")
        self.loop.run()
...
def main():
    ...
    if not _acquire_single_instance():
        print("WhisperType is already running (single instance).")
        # Bring attention via notification; don't start a second copy.
        try:
            subprocess.Popen(
                ["notify-send", "-i", "audio-input-microphone", "WhisperType",
                 "WhisperType is already running — check the tray."],
    ...
    if errors:
        print("FAILED")
        for e in errors:
            print(f"  - {e.replace(chr(10), ' ')}")
        show_error_dialog("\n\n".join(errors))
        sys.exit(1)
    print("OK")

    # If the model isn't cached yet, download it with a visible progress dialog.
    if whisper.model_needs_download(whisper.MODEL_SIZE):
        print(f"Model '{whisper.MODEL_SIZE}' not cached — downloading with progress UI...")
        dialog, progress_cb = show_download_progress_dialog(whisper.MODEL_SIZE)
        ...
        def _download_job():
            try:
                ...
                # We marshal via GLib.idle_add in a small adapter.
                def cb(fraction, bytes_done):
                    GLib.idle_add(progress_cb, fraction, bytes_done)
                whisper.download_model(whisper.MODEL_SIZE, progress_cb=cb)
                result["ok"] = True
            except Exception as e:  # noqa: BLE001
                result["error"] = e
            # Schedule dialog close on the main thread (guard against double-close).
            def _close():
        ...
        # Run the dialog; it auto-closes when the download thread finishes.
        dialog.run()
        dialog_done["closed"] = True
        dialog.destroy()
        ...
        if not result["ok"]:
            if result["error"] is None:
                # user closed the dialog before download finished
    ...
    try:
        app = DictationApp(model)
        app.run()
    except SystemExit:
        pass
    except Exception as e:
        print(f"[ERROR] Startup failed: {e}")
        show_error_dialog(f"WhisperType failed to start:\n\n    {e}")
    finally:
        print("\nExiting.")
