//! whispertype-helperd — privileged paste daemon for WhisperType.
//!
//! Runs as root (via systemd). Owns the ONE privileged action WhisperType needs:
...
//! daemon over a Unix socket at /run/whispertype.sock. Peers are authorized by
...
const SOCK_PATH: &str = "/run/whispertype.sock";
...
impl UInput {
    fn open() -> Result<Self, String> {
        ...
        let name = b"WhisperType Virtual Keyboard\0";
...
fn main() {
    let listen_fd = match open_listen_socket() {
        Ok(fd) => fd,
        Err(e) => {
            eprintln!("whispertype-helperd: {e}");
            std::process::exit(1);
        }
    };
    ...
    let uinput = match UInput::open() {
        Ok(u) => Some(u),
        Err(e) => {
            eprintln!("whispertype-helperd: {e}; Ctrl+V injection disabled");
            None
        }
    };
    ...
    eprintln!(
        "whispertype-helperd: ready ({}) — waiting for paste requests",
    ...
    loop {
        ...
        if rc < 0 {
            ...
            eprintln!("whispertype-helperd: poll: {}", strerror_str(errno()));
            break;
        }
        if rc == 0 {
            continue;
