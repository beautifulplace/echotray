"""Client for the privileged whispertype-helperd daemon.

The WhisperType GUI runs as an unprivileged user and has NO access to
/dev/uinput. All privileged input (Ctrl+V injection) is handled by root-owned
whispertype-helperd, which we talk to over a Unix socket at
/run/whispertype.sock.
...
SOCKET_PATH = "/run/whispertype.sock"
