...
whispertype-helperd daemon. This module handles microphone capture, Whisper
...
MODEL_CACHE_ROOT = os.getenv(
    "MODEL_DIR", os.path.join(os.path.expanduser("~"), ".local", "share", "whispertype", "models")
)
...
def _download_file(url, dest, progress_cb=None, chunk_size=1 << 20):
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "whispertype/3.2"})
...
def load_model():
    # WhisperType always runs on CPU (int8). No GPU/device selection — this keeps
    # it working identically on any machine, including ones with an NVIDIA GPU
    # where CUDA/float16 can cause problems.
    device = "cpu"
    compute_type = COMPUTE_TYPE
    if compute_type == "float16":
        print("  Warning: float16 requires a CUDA GPU. WhisperType runs on CPU — using int8.")
        compute_type = "int8"

    print(f"Loading Whisper model ({MODEL_SIZE}, device=cpu, compute={compute_type})... ", end="", flush=True)
    start = time.monotonic()

    # Load from our local cache if present; otherwise let faster-whisper download.
    local_dir = _model_local_dir(MODEL_SIZE)
