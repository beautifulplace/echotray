...
def testenv_int_default_when_unset(monkeypatch):
    monkeypatch.delenv("ET_TEST_INT", raising=False)
    assert whisper.env_int("ET_TEST_INT", 42) == 42


def testenv_int_parses_valid(monkeypatch):
    monkeypatch.setenv("ET_TEST_INT", "7")
    assert whisper.env_int("ET_TEST_INT", 42) == 7


def testenv_int_invalid_falls_back(monkeypatch, capsys):
    monkeypatch.setenv("ET_TEST_INT", "not-a-number")
    assert whisper.env_int("ET_TEST_INT", 42) == 42
    assert "[config]" in capsys.readouterr().err


def testenv_int_empty_string_falls_back(monkeypatch):
    monkeypatch.setenv("ET_TEST_INT", "   ")
    assert whisper.env_int("ET_TEST_INT", 42) == 42


def testenv_int_clamps_bounds(monkeypatch):
    monkeypatch.setenv("ET_TEST_INT", "1")
    assert whisper.env_int("ET_TEST_INT", 42, lo=5, hi=10) == 5
    monkeypatch.setenv("ET_TEST_INT", "99")
    assert whisper.env_int("ET_TEST_INT", 42, lo=5, hi=10) == 10
