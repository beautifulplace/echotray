...
**Runs unprivileged.** A root-owned helper daemon (`whispertype-helperd`) owns the
...
root-owned Unix socket (`/run/whispertype.sock`). See **Architecture** below.
...
root-owned daemon. The interaction model (a simple click-to-talk microphone) is
inspired by [woheller69/whisperIMEplus](https://github.com/woheller69/whisperIMEplus).

## How It Works

1. WhisperType lives in the tray as a small microphone icon (green = ready).
...
WhisperType is split into two processes to minimize the privileges needed by the GUI:

```
┌─────────────────────────────┐        ┌──────────────────────────────┐
│  whispertype (GUI)          │        │  whispertype-helperd (root)   │
...
WhisperType needs a **Wayland** compositor and **AppIndicator** tray support. It
...
> **Note:** WhisperType is currently tested on **GNOME/Mutter only**. The KDE and
...
```bash
git clone https://github.com/beautifulplace/whispertype.git
cd whispertype
chmod +x setup.sh
./setup.sh
```
...
- Builds and installs the helper daemon (`whispertype-helperd`) as a systemd service
  that runs as root
- Installs the app into `~/.local/share/whispertype/` (app code, its venv, config,
  and the downloaded model), creating a Python venv and installing dependencies
- Installs a `.desktop` launcher

Everything WhisperType owns lives under **`~/.local/share/whispertype/`**:
- `app/` — the app source
...
Your configuration lives at **`~/.local/share/whispertype/.env`** (created on setup
from the template). Edit it to your preferences:

```bash
nano ~/.local/share/whispertype/.env
```
...
| `MODEL_DIR` | `~/.local/share/whispertype/models` | Where the Whisper model is stored |

## Usage

**From the app menu:** Search for "WhisperType" in Activities / app launcher.

**From the terminal:**
```bash
~/.local/share/whispertype/venv/bin/python ~/.local/share/whispertype/app/src/app.py
```

1. Wait for the "Ready" notification (first run downloads the Whisper model)
2. Click the tray microphone to start recording (turns **red**)
...
5. Click it again to stop; the transcript is pasted at your cursor
   (turns **amber** while transcribing)
...
| Error | Fix |
|-------|-----|
| `whispertype-helperd is not running` | `sudo systemctl start whispertype-helperd`, or re-run `./setup.sh` |
...
MIT. This project is an adaptation of
...
- **whisperIMEplus** by woheller69 — the interaction model (a simple click-to-talk
  microphone) that inspired the design. No code is used from it.
